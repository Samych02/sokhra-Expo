    ...
    <Card containerStyle={{
      ...
      <View className="flex-row items-center justify-between mb-3">
        <Text className="font-pbold text-xl text-black" style={{width: "40%"}} numberOfLines={1}
              ellipsizeMode="tail">{capitalizeFirstLetter(item.origin.city)}</Text>
        ...
        <Text className="font-pbold text-xl text-black text-right" style={{width: "40%"}}
              numberOfLines={1}
              ellipsizeMode="tail">{capitalizeFirstLetter(item.destination.city)}</Text>
      </View>

      <View className="flex-row mb-2 justify-start">
        <MaterialCommunityIcons name="calendar-import" size={15} color={COLORS.cgrey}/>
        <Text className="font-psemibold text-black text-xs"> Départ le {item.departureDate}</Text>
      </View>

      <View className="flex-row items-center justify-between mb-2">
        <View className="flex-row">
          <MaterialCommunityIcons name="weight" size={15} color={COLORS.cgrey}/>
          <Text className="font-psemibold text-black text-xs"> {item.availableWeight} kg disponible</Text>
        </View>
      ...
      <View className="flex-row justify-between ">
        <TouchableOpacity className="flex-row items-center" onPress={() => {
          ...
          <View className="flex-col ml-4 h-full">
            <Text className="font-psemibold text-black ">{capitalizeFirstLetter(item.traveller.firstName)}</Text>
            <View className="flex-row items-center">
              <Rating rating={item.traveller.rating.ratingValue} size={14} scale={1} spacing={3} disabled={true}
                      baseColor={COLORS.cgrey}/>
              <Text className="text-cgrey text-xs ml-1">({item.traveller.rating.numberOfRatings})</Text>
            </View>
          </View>
        </TouchableOpacity>
